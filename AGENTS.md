...
- Build error note: when calling `MethodInfo.Invoke(...)` in tests, wrap array arguments in an explicit `object[]` (e.g. `new object[] { new Tensor[] { ... } }`); nested collection expressions can target `object` and trigger CS9174.
- Build error note: `IndexType` lives in the `NRedberry` namespace (from `NRedberry.Core.Entities/IndexType.cs`), not `NRedberry.Core.Entities`; add `using NRedberry;` when tests reference the enum to avoid CS0234.
