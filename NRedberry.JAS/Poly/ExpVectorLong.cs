...
public sealed class ExpVectorLong : ExpVector
{
    ...
    public override ExpVector Extend(int count, int index, long exponent)
    {
        ...
        long[] result = new long[_values.Length + count];
    ...
    public override ExpVector ExtendLower(int count, int index, long exponent)
    {
        ...
        result[_values.Length + index] = exponent;
        return new ExpVectorLong(result);
    }
    ...
    public override ExpVector Combine(ExpVector vector)
    {
        ...
        long[] result = new long[_values.Length + other.Length];
        Array.Copy(_values, 0, result, 0, _values.Length);
    ...
    private static long[] ExtractValues(ExpVector vector)
    {
        if (vector is ExpVectorLong longVector)
        {
            return longVector._values;
        }

        return vector.GetVal();
    }

    private void EnsureSameLength(long[] other, string parameterName)
    {
